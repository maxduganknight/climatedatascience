...
def create_average_premium_chart(df, title_suffix, save_path, custom_title=None, custom_subtitle=None, use_inflation_adjusted=True):
    ...
    # Use custom title/subtitle if provided, otherwise use defaults
    title = custom_title if custom_title else f"FLORIDA HOMEOWNERS INSURANCE AVERAGE PREMIUM"
    subtitle = custom_subtitle if custom_subtitle else f"AVERAGE ANNUAL PREMIUM PER POLICY {title_suffix_adj.upper()}"
    data_note = f"DATA: FLORIDA OFFICE OF INSURANCE REGULATION | OWNER-OCCUPIED RESIDENTIAL HOMEOWNERS (EXCL. TENANT AND CONDO)"
...
def create_policies_with_new_policies_chart(df, title_suffix, save_path, custom_title=None):
    """Create a dual-axis chart showing policies in force (bars) and new policies written (line).
    ...
    custom_subtitle : str, optional
        Custom subtitle to override the default
    """
    # Set up the plot with common styling and wider figure for dual axes
    fig, ax1, font_props = setup_enhanced_plot(figsize=(14, 8))

    # Create bars for policies in force (secondary axis - left side)
    ax2 = ax1.twinx()

    # Convert policies to thousands for better display
    policies_thousands = df['Policies in force'] / 1000

    bars = ax2.bar(df['year'], policies_thousands,
                  color=COLORS['comparison'],
                  alpha=0.6,
                  width=0.6,
                  label='Policies in Force')

    # Plot new policies written line on primary axis (right side)
    new_policies_thousands = df['Number of new policies written'] / 1000
    line = ax1.plot(df['year'], new_policies_thousands,
    ...
    # Use custom title/subtitle if provided, otherwise use defaults
    title = custom_title if custom_title else "FLORIDA HOMEOWNERS INSURANCE POLICIES"
    # subtitle = custom_subtitle if custom_subtitle else f"POLICIES IN FORCE AND NEW POLICIES WRITTEN {title_suffix.upper()}"
    data_note = "DATA: FLORIDA OFFICE OF INSURANCE REGULATION | OWNER-OCCUPIED RESIDENTIAL HOMEOWNERS (EXCL. TENANT AND CONDO)"

    # Format the plot title
    format_plot_title(ax1, title, '', font_props)
    ...
    # Primary y-axis (right side) - New policies written
    ax1.yaxis.set_major_formatter('{x:,.0f}K')
    ax1.tick_params(axis='y', labelsize=14, colors=COLORS['primary'])
    ax1.set_ylabel('New Policies Written', color=COLORS['primary'], fontproperties=font_prop, fontsize=14)
    ax1.yaxis.set_label_position('right')
    ax1.yaxis.tick_right()

    # Secondary y-axis (left side) - Policies in force
    ax2.yaxis.set_major_formatter('{x:,.0f}K')
    ax2.tick_params(axis='y', labelsize=14, colors=COLORS['comparison'])
    ax2.set_ylabel('Policies in Force', color=COLORS['comparison'], fontproperties=font_prop, fontsize=14)
    ...
    if bars_artist:
        ...
        legend_labels = ['Policies in Force', 'New Policies Written']
    ...
    # Save the plot if a path is provided with tight bounding box
    if save_path:
...
def create_citizens_premium_transferred_chart(df, save_path, custom_title=None, custom_subtitle=None):
    ...
    line1 = ax1.plot(df['year'], df['average_premium'], 
                     color=color1, 
                     linewidth=3, 
                     marker='o', 
                     markersize=8,
    ...
    # Use custom title/subtitle if provided, otherwise use defaults
    title = custom_title if custom_title else "CITIZENS PROPERTY INSURANCE PREMIUMS AND POLICIES IN FORCE"
    subtitle = custom_subtitle if custom_subtitle else "AVERAGE ANNUAL PREMIUM AND NUMBER OF POLICIES IN FORCE"
    data_note = "DATA: FLORIDA OFFICE OF INSURANCE REGULATION | OWNER-OCCUPIED RESIDENTIAL HOMEOWNERS (EXCL. TENANT AND CONDO)"
...
def main():
    ...
    # Define data paths
    all_insurers_path = 'insurance/florida_homeowners_insurance.csv'
    citizens_path = 'insurance/florida_homeowners_insurance_citizens.csv'
    
    # Create output directory
    output_dir = 'figures/insurance'
    ...
    all_insurers_df.to_csv("insurance/all_insurers_output.csv")
    ...
    # 1. Combined policies chart - policies in force (bars) and new policies written (line)
    create_policies_with_new_policies_chart(
        all_insurers_df,
    ...
    create_average_premium_chart(
        all_insurers_df,
        "across all Florida insurers",
        f"{output_dir}/fl_all_insurers_average_premium.png",
        custom_title="FLORIDA HOME INSURANCE COSTS INCREASING",
    ...
    create_average_premium_chart(
        citizens_df,
        "for Citizens Property Insurance Corporation",
        f"{output_dir}/fl_citizens_average_premium.png",
        custom_title="EVEN THE STATE INSURER-OF-LAST-RESORT IS NOW UNAFFORDABLE (2020 DOLLARS)",
        use_inflation_adjusted=True
    )
    
    # 5b. Average premium with transferred policies
    create_citizens_premium_transferred_chart(
        citizens_df,
        f"{output_dir}/fl_citizens_premium_transferred.png"
    )
    
    # 6. Combined policies chart for Citizens
    # create_policies_with_new_policies_chart(
    #     citizens_df,
    #     "for Citizens Property Insurance Corporation",
    #     f"{output_dir}/fl_citizens_new_policies_written.png",
    #     custom_title="CITIZENS BECOMES THE DOMINANT FLORIDA HOME INSURER",
    #     custom_subtitle="POLICIES IN FORCE AND NEW POLICIES WRITTEN FOR CITIZENS"
    # )
