...
SEAS5_events <- read.csv("../../data/UNSEEN/wildfire/cems/preprocessed/SEAS5_CA_events.csv")
ERA5_events <- read.csv("../../data/UNSEEN/wildfire/cems/preprocessed/ERA5_CA_events.csv")
...
UNSEEN_bc$fwinx <- (UNSEEN_bc$fwinx +
                    mean(ERA5_events_hindcast$fwinx) - mean(SEAS5_events_hindcast$fwinx)
)
...
timeseries = unseen_timeseries(
  ensemble = UNSEEN_bc,
  obs = obs,
  ensemble_yname = "fwinx",
  ensemble_xname = "year",
  obs_yname = "fwinx",
  obs_xname = "year",
  ylab = "May, June, July, August CA FWI")
