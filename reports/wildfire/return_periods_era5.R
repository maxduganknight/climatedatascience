...
# era5_p95_count <- era5_p95_count[order(Year), ]


severity_plot <- ggplot(era5_yearly_max, aes(x = year, y = fwinx)) +
  geom_line() +  # Line plot
...
log_likelihood_severity <- gev_fit_accelerating$results$value
aic_severity <- summary(gev_fit_accelerating)$AIC
bic_severity <- summary(gev_fit_accelerating)$BIC
...
frequency_min_index <- min(era5_p95_count$year_index)
frequency_max_index <- max(era5_p95_count$year_index)

frequency_qcov_start <- make.qcov(frequency_model, vals = list(mu1 = frequency_min_index, mu2 = frequency_min_index^2))
